        ...
        <div className="bg-white w-full max-w-3xl rounded-[2.5rem] shadow-2xl overflow-hidden animate-zoom-in">
          ...
          <div className="p-6 max-h-[60vh] overflow-y-auto space-y-4 bg-gray-50/50">
            {localAppointments.length > 0 ? (
                  ...
                  <div key={app.id} className="bg-white p-6 rounded-[2rem] border border-gray-100 shadow-sm flex items-center justify-between group hover:border-ibicuitinga-royalBlue/30 transition-all">
                    <div className="space-y-2">
                      <div className="flex items-center gap-2">
                        <span className="bg-ibicuitinga-navy text-white text-[9px] font-black px-2 py-1 rounded-md uppercase">{app.protocol}</span>
                        <span className={`text-[9px] font-black px-2 py-1 rounded-md uppercase ${
                          ...
                          {app.status === 'cancelled' ? 'Cancelado' : app.status === 'completed' ? 'Concluído' : 'Ativo'}
                        </span>
                      </div>
                      <h4 className="text-xl font-black text-ibicuitinga-navy capitalize">{app.citizen.name}</h4>
                      <p className="text-[10px] font-bold text-gray-400 uppercase tracking-tight">
                        {/* Correção de Data: Split para evitar fuso horário */}
                    ...
                    <div className="flex gap-3">
                      <button 
                        ...
                        className={`p-3 rounded-full transition-all shadow-sm active:scale-95 ${
                          isActionDisabled ? 'bg-gray-100 text-gray-300 cursor-not-allowed opacity-50' : 'bg-ibicuitinga-skyBlue/10 text-ibicuitinga-royalBlue hover:bg-ibicuitinga-royalBlue hover:text-white'
                        }`}
                      >
                        <Pencil size={18} />
                      </button>
                      <button 
                        ...
                        className={`p-3 rounded-full transition-all shadow-sm flex items-center justify-center active:scale-95 ${
                          isActionDisabled ? 'bg-gray-100 text-gray-300 cursor-not-allowed opacity-50' : 'bg-red-50 text-red-500 hover:bg-red-500 hover:text-white'
                        }`}
                      >
              ...
              <div className="py-20 text-center text-gray-300 font-black uppercase tracking-widest text-xs">Nenhum registro encontrado.</div>
            )}
          </div>
        </div>
      </div>
